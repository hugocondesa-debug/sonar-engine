...
### CAL-TEST-CYCLES-FIXTURE-FLAKE-AND-FAIL — Cycles US tests inconsistent (CLOSED 2026-04-26 via Sprint A)
- **Priority:** LOW → **CLOSED 2026-04-26** via Sprint A test-hygiene + SESSION_STATE.md hybrid governance.
...
- **Issue 1 (genuine failure) — FIXED:** `tests/integration/test_cycles_composites.py::TestOrchestratorSmoke::test_us_smoke_end_to_end` fails consistently on main + branches. Error: `cycles.msc.skipped error="Composite requires >= 3 sub-indices; got 0 (missing: ['CS', 'M1', 'M2', 'M3', 'M4'])"`. Root cause: `_seed_all` seed function not populating in-memory test session for US monetary subindices. Schema canonical: `monetary_m1_effective_rates` / `monetary_m2_taylor_gaps` / `monetary_m4_fci`. **Fix shipped Commit 2 c5810c8**: `_seed_all` extended with `_seed_monetary_subindices` (M1+M2+M4 dedicated tables + M3 IndexValue) + `_seed_economic_subindices` (E1+E3+E4 dedicated tables + E2 IndexValue). Verified isolated PASS + full-suite PASS x5 consecutive.
- **Issue 2 (flake) — NOT-REPRODUCIBLE post-Issue 1 fix:** `tests/unit/test_cycles/test_financial_fcs.py::TestComputeFcsHappy::test_us_full_stack` passes isolated (`pytest path::test_id`) but fails in full-suite run. Order-dependent test pollution / shared fixture interaction. **Investigation outcome (Commit 3 49805c0)**: 5x consecutive full-suite runs post-Issue 1 fix → 0/5 falhas. Diagnóstico: Issue 2 era sintoma de Issue 1 (knock-on effect via pytest collect/order pollution apparente quando smoke test falhava). Sem patch defensiva adicional ao `_seed_f_rows` / `db_session` fixture (function-scoped + `expire_on_commit=False` + explicit teardown já são best-practice).
- **Required work — DONE:**
  1. ✅ Audit `_seed_all` (`test_cycles_composites.py`) seed function vs canonical schema names — Commit 1 audit doc `docs/planning/week11-sprint-a-pre-flight-audit.md`.
  2. ✅ Audit `_seed_f_rows` (`test_financial_fcs.py`) for shared state with sibling tests — same audit doc §4.
  3. ✅ Add per-test session isolation (function-scoped fixtures) where needed — confirmed already function-scoped; sem patch necessária.
  4. ✅ Verify composite readers (`compute_all_cycles`, `compute_fcs`) match seed naming — audit doc §2: zero divergence.
- **Test runs evidence (post-fix Commit 2 c5810c8 + Commit 3 49805c0):**
  - `test_us_smoke_end_to_end`: PASS isolated (1/1, 0.62s); PASS full-suite x5 (5/5).
  - `test_us_full_stack`: PASS full-suite x5 (5/5) — Issue 2 NOT-REPRODUCIBLE.
- **Pre-existing flakes surfaced (out-of-scope) — CLOSED 2026-04-26 via Sprint 8 (`b37b29e`):** `test_te_indicator.py` cassette tests (CA / SE / EA / FR various — surfaced as `cumulative_calls` bleed mas root cause distinto); `test_economic_ecs::test_fixture_us_2020_03_23_recession` + `::test_fixture_hysteresis_whipsaw_reject` (intermittent ~1/5); `test_credit_cccs::TestComputeCccsEndToEnd::test_happy_full_stack` (intermittent). Diagnóstico Sprint 8 (PYTHONTRACEMALLOC=10): pytest-asyncio's `_get_event_loop_no_warn` (`plugin.py:618`) chama `asyncio.get_event_loop()` que sob Python 3.12.3 default policy auto-cria orphan `_UnixSelectorEventLoop` quando `_local._set_called == False`. Cada loop órfão carrega 2 sockets self-pipe; GC eventualmente emite ResourceWarning → PytestUnraisableExceptionWarning → ERROR via `filterwarnings`, falhando teste random downstream. Fix: autouse session-scoped fixture em `tests/conftest.py` define `policy._local._set_called = True` (b37b29e); pytest-asyncio's existente RuntimeError catch em `_temporary_event_loop_policy` preserva o contract. Verificação 5x consecutive full-suite post-fix: 5/5 PASS clean (2322 passed). Hipótese inicial "TE rate-limit bleed" falsified — flakes eram sintoma generic do orphan-loop GC, não específicas de TE.
- **Impact resolved:** Pre-push gate consistent failure pattern eliminado para os 2 testes alvo; option-2 push-and-track pattern não mais necessário para estes 2 testes específicos. Sprint 8 closure: residual full-suite flakes também eliminados (5/5 clean post-fix em `b37b29e`).
- **Estimate vs actual:** 2-3h estimate → ~2h wall-clock single CC (Track 1 Commits 1-4). Sprint 8 follow-up closure: ~1h adicional (Commits 1-3 deste sprint).
- **Related:** Sprint 7B Commit 1 (defer-and-track precedent); Sprint A brief `docs/planning/week11-sprint-a-test-hygiene-and-session-state-brief.md`; pre-flight audit `docs/planning/week11-sprint-a-pre-flight-audit.md`; Issue 2 investigation `docs/planning/week11-sprint-a-issue-2-investigation.md`; Sprint 8 brief `docs/planning/week11-sprint-8-test-pollution-rootcause-brief.md`; Sprint 8 hypothesis ranking `docs/planning/week11-sprint-8-hypothesis-ranking.md`; Sprint Q.0.5 retro Discovery #3 (precedente conhecido do mesmo padrão, fix parcial via `_stub_main` em `test_daily_monetary_indices.py`).
