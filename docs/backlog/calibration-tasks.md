...
- **Current behavior:** ``daily_curves --country IT`` raises ``InsufficientDataError`` citing ``CAL-CURVES-IT-BDI``; ``--all-t1`` skips. ``BancaDItaliaConnector`` instantiable via standard constructor kwargs; every fetch method short-circuits with the probe matrix.
...
- **Estimate to re-attempt:** 4-6h CC *after* any of the three unblock conditions materialise. Probe-matrix refresh is the mandatory first step (Infostat subdomain DNS state; BdI website; licensed-feed access).
- **Related:** ``CAL-CURVES-EA-PERIPHERY`` (superseded grandparent); ``CAL-CURVES-FR-BDF`` (BLOCKED pilot — Sprint D template); ``CAL-CURVES-ES-BDE`` (Sprint G twin — HALT-0 via different sub-case); ``ADR-0009`` (national-CB probe pattern + pattern library entry); ``CAL-CURVES-T1-LINKER`` IT BTP€i portion absorbed here.
...
- **Current behavior:** ``daily_curves --country ES`` raises ``InsufficientDataError`` citing ``CAL-CURVES-ES-BDE``; ``--all-t1`` skips. ``BancoEspanaConnector`` instantiable via standard constructor kwargs; every fetch method short-circuits with the probe matrix. Unlike the IT / FR HALT-0 cases, the ES scaffold's docstring preserves the concrete series-code shape (``D_`` prefix; BE_22 statistical-table chapter) for future swap-in.
- **Impact if unresolved:** ES overlays on EA-aggregate proxy; linker path limited to DERIVED BEI fallback (Bonos indexados coverage weakest among EA-periphery peers). Peripheral spread signal retains FR + IT gaps from Sprints D + G.
- **Estimate to re-attempt:** 4-6h CC under (a) if BdE publishes daily yields; 6-8h CC under (b) because adding monthly-cadence support is a pipeline-wide change (staleness semantics, overlay-cascade implications, backtest re-baselining). Probe-matrix refresh mandatory first step.
- **Related:** ``CAL-CURVES-EA-PERIPHERY`` (superseded grandparent); ``CAL-CURVES-FR-BDF`` (BLOCKED pilot — Sprint D template); ``CAL-CURVES-IT-BDI`` (Sprint G twin — HALT-0 via strict all-paths-dead sub-case); ``ADR-0009`` (national-CB probe pattern + pattern library entry with the "non-daily" sub-case); ``CAL-CURVES-T1-LINKER`` ES Bonos-i portion absorbed here.
