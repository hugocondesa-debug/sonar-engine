...
### CAL-CURVES-FR-TE-PROBE — FR sovereign yield curve re-probe via TE per-tenor cascade

- **Priority:** MEDIUM — FR is the fourth-largest EA economy and the widest EA-core spread gap; country-specific curve would replace EA-aggregate-proxy for FR ERP/CRP overlays.
- **Status:** OPEN — opened Week 10 Sprint H 2026-04-22 as a direct consequence of the ADR-0009 v2 amendment (TE Path 1 canonical). CAL-138 2026-04-22 probe noted "TE `GFRN10` 10Y-only, below MIN_OBSERVATIONS=6" but that probe **did not sweep per-tenor** the `GFRN{1M, 3M, 6M, 1Y, 2Y, 3Y, 5Y, 7Y, 10, 15, 20, 30}` spectrum. The Sprint H IT + ES success reveals that TE's Bloomberg-symbol coverage is frequently richer than the country-indicator slug suggests; FR warrants a Sprint H-style per-tenor sweep before defaulting to the BdF national-CB scaffold path.
- **Trigger:** ADR-0009 v2 Sprint H addendum — "any country-data probe sprint MUST include TE generic indicator API as Path 1 in pre-flight probe matrix. National CB probe only when TE coverage gap confirmed empirically."
- **Pre-flight discipline:** Sprint H brief §2 per-tenor sweep script applies directly. Scope: single country (FR), 12 candidate tenors (`GFRN1M:IND` … `GFRN30Y:IND` with suffix variants per CAL-138 GB/JP/CA + Sprint H IT/ES quirks). Probe budget: ~1h CC.
- **Candidate path:** TE `/markets/historical` with per-tenor Bloomberg symbols. Known single-point (`GFRN10:IND`) from `TE_10Y_SYMBOLS`; remaining 11 tenors unverified.
- **Required work:**
  - If Sprint H-style probe confirms ≥ 6 tenors daily: extend `TE_YIELD_CURVE_SYMBOLS["FR"]` and remove FR from `_DEFERRAL_CAL_MAP`. Budget ~2h CC (pattern replication, 7th use after Sprint H).
  - If probe fails (< 6 tenors OR non-daily majority): document the empirical evidence in a sharpened sibling entry on this CAL item + close with "TE insufficient"; `CAL-CURVES-FR-BDF` national-CB scaffold remains the future unblock path.
- **Impact if unresolved:** FR overlays continue on EA-aggregate proxy; rating-spread signal retains the FR component of the periphery gap (currently the largest remaining unmodeled spread after IT + ES closure).
- **Estimate:** 2h CC (probe + ship if Path 1 succeeds) to 4h CC (probe + documented scaffold reaffirmation if Path 1 fails).
- **Related:** ``CAL-CURVES-FR-BDF`` (BLOCKED national-CB pilot — stays BLOCKED regardless of this sprint's outcome); ``CAL-CURVES-IT-BDI`` + ``CAL-CURVES-ES-BDE`` (Sprint H precedent — same correction applied to FR); ``ADR-0009`` v2 (Sprint H amendment that mandates this re-probe); ``CAL-138 retrospective`` (original single-tenor finding for FR).
