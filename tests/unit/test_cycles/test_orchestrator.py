...
from sonar.db.models import Base, CreditCycleScore, FinancialCycleScore, MonetaryCycleScore
...
class TestOrchestrator:
    ...
    def test_partial_missing_returns_three_skips(self, db_session: Session) -> None:
        # Nothing seeded — all three cycles skip.
    ...
    def test_full_stack_with_msc_produces_all_three(self, db_session: Session) -> None:
        _seed_full_stack(db_session)
    ...
    def test_msc_only_skips_when_sub_indices_absent(self, db_session: Session) -> None:
        # Seed only MSC sub-indices → CCCS + FCS skip; MSC produces a row.
        _seed_msc_inputs(db_session)
        outcome = compute_all_cycles(db_session, "US", date(2024, 1, 31))
        assert outcome.cccs is None
        assert outcome.fcs is None
